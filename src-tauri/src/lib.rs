...
fn resolve_data_root(app: &AppHandle) -> PathBuf {
    for p in pointer_candidates(app) {
        if let Ok(s) = fs::read_to_string(&p) {
            ...
            // A pointer holding exactly the COMPUTED default is residue of the
            // removed first-run lock (see setup()), not a user choice — drop it
            // so the default stays live. A real Change-folder selection never
            // equals the default and keeps winning. Family-wide 2026-08-14.
...
pub fn run() {
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            // first probe. NO first-run pointer lock (family ruling 2026-08-14, in
            // lock-step with JustWrite and JustVoice): the default is COMPUTED every
            // boot, so a later default change actually takes effect. dataroot.txt
            // records ONLY an explicit Change-folder — in JustVoice the first-run lock
            // had pinned installs to an obsolete default and vetoed the new one.
            ...
            // window; the menu drives the server. The default app icon is the tray icon.
            let menu = build_tray_menu(app.handle())?;
            let _tray = TrayIconBuilder::with_id("just-ai-i18n-docgen-tray")
                .tooltip("Just AI i18n & DocGen")
        ...
        .invoke_handler(tauri::generate_handler![storage_get_root, storage_relocate, pick_directory, set_keep_server_running])
        .on_window_event(|window, event| {
            if let WindowEvent::CloseRequested { api, .. } = event {
                // The family headless ruling (2026-08-04): keep-running ON ⇒ the
                // window hides to the tray and the server stays; OFF ⇒ closing
                // stops everything (the sidecar dies with the app).
